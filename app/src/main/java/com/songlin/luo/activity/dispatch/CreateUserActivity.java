...
public class CreateUserActivity extends Activity implements OnClickListener {
	...
	private void hanleCreateAccountResult(JSONObject json) {
		...
		try {
			...
			Toast.makeText(this, "没有获取网络的响应", Toast.LENGTH_LONG).show();
			e.printStackTrace();
			return;
		}
		...
		if(result == 2) {
			Toast.makeText(this, "注册失败！服务器端出现异常", Toast.LENGTH_LONG).show();
			return;
		}
		
		if(result == 0) {
			Toast.makeText(this, "注册成功！将前往登录界面...", Toast.LENGTH_LONG).show();
	...
	private void handleCreateAccount() {
		...
		if(!isUsernameValid) {
			Toast.makeText(this, "用户名不正确，请重新请输入", Toast.LENGTH_LONG).show();
			return;
		}
		...
		if(isAgeValid == -3) {
			Toast.makeText(this, "年龄格式错误", Toast.LENGTH_LONG).show();
			return;
		}
